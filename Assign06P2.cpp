...
int main()
{
   ...
   do
   {
      ...
      portToArrayInOrder(bst_root, portOri);
      if (! match(dataOriSorted, portOri, oriCount) )
      {
         cout << "Insert error ... bad in-order traversal output" << endl;
         cout << "expected: ";
         ShowArray(dataOriSorted, oriSize);
         cout << "got this: ";
         ShowArray(portOri, oriSize);
         exit(EXIT_FAILURE);
      }
      ...
      {
         portPostRem = new int [used];
         portToArrayInOrder(bst_root, portPostRem);
         if (! match(dataOriSorted, portPostRem, used) )
         {
            cout << "Remove error ... bad in-order traversal output" << endl;
            cout << "expected: ";
            ShowArray(dataOriSorted, used);
            cout << "got this: ";
            ShowArray(portPostRem, used);
            exit(EXIT_FAILURE);
         }
      }
      ...
      delete [] portPostRem;
      dataOriSorted = portOri = dataRemRandom = portPostRem = 0;
   }
